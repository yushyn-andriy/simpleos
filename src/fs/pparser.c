...
{
    int len = strnlen(filename, SIMPLEOS_MAX_PATH);
    return (len >= 3 && isdigit(filename[0]) && memcmp((void*)&filename[1], ":/", 2) == 0);
}

// this function modifies the path given as an argument
...
static struct path_root* pathparser_create_root(int drive_numebr)
{
    struct path_root *path_r = kzalloc(sizeof(struct path_root));
    path_r->drive_no = drive_numebr;
    path_r->first = 0;
    return path_r;
}
...
{
    char* result_path_part = kzalloc(SIMPLEOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > SIMPLEOS_MAX_PATH)
    {
        goto out;
    }
